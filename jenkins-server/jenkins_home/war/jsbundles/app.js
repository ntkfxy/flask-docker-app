...
function init() {
  behavior_shim.specify("#auto-overflow", "-dropdowns-", 1000, element => {
    const template = JSON.parse(element.nextSibling.content.textContent);
    const appBarItems = utils.mapChildrenItemsToDropdownItems(template.items.filter(e => e.group.order <= 2));
    const overflowItems = utils.mapChildrenItemsToDropdownItems(template.items.filter(e => e.group.order > 2));
    ...
    appBarItems.forEach((item, index) => {
      ...
      element.parentNode.insertBefore(templates.menuItem(item, "jenkins-button", template.url), element);
    });

    // Add any additional items as an overflow menu
    if (overflowItems.length > 0) {
      utils.generateDropdown(element, instance => {
        instance.setContent(utils.generateDropdownItems(overflowItems, false, template.url));
      });
    }
  });
}
...
function generateDropdowns() {
  ...
  behavior_shim.specify("li.children, .jenkins-jumplist-link, #menuSelector, .jenkins-menu-dropdown-chevron", "-dropdown-", 1000, element => utils.generateDropdown(element, instance => {
    ...
    fetch(path.combinePath(href, jumplistType)).then(response => response.json()).then(json => instance.setContent(utils.generateDropdownItems(utils.mapChildrenItemsToDropdownItems(json.items), false, href))).catch(error => console.error(`Jumplist request failed:`, error)).finally(() => instance.loaded = true);
  }));
}
  ...
  return instance => {
    ...
    const fetchSection = function (urlSuffix) {
      return fetch(path.combinePath(href, urlSuffix)).then(response => response.json()).then(json => {
        const items = utils.generateDropdownItems(jumplists_mapChildrenItemsToDropdownItems(json.items));
        return items;
      });
    };
    const promises = [];
 ...
 * Generates the contents for the dropdown
 */
function jumplists_mapChildrenItemsToDropdownItems(items) {
  return items.map(item => {
    ...
    return {
      ...
      subMenu: item.subMenu ? () => {
        return jumplists_mapChildrenItemsToDropdownItems(item.subMenu.items);
      } : null
    };
  });
}
